    ...
    suspend fun uploadAnonymousAssessment(
        ...
        withContext(Dispatchers.IO) {
            ...
            try {
                ...
                Log.d("UPLOAD_DEBUG", "Video file exists: ${assessmentData.videoFile?.exists()}")
                ...
                Log.d("UPLOAD_DEBUG", "GAD-7 Score: ${assessmentData.gad7Score}")  // ✅ Log score
                Log.d("UPLOAD_DEBUG", "Questionnaire Score: ${assessmentData.questionnaireScore}")  // ✅ Log score


                assessmentData.videoFile?.let {
                    Log.d("UPLOAD_DEBUG", "Video size: ${it.length()} bytes")
                }

                assessmentData.auCsvFile?.let {
                    Log.d("UPLOAD_DEBUG", "AU CSV size: ${it.length()} bytes")
                }

                assessmentData.gad7CsvFile?.let {
                    Log.d("UPLOAD_DEBUG", "GAD7 CSV size: ${it.length()} bytes")
                }
...
addFormField(writer,boundary,"registration_id",assessmentData.registrationId)
                ...
                addFormField(writer, boundary, "assessment_type", assessmentData.assessmentType)
                // Add scores as form fields
                addFormField(writer, boundary, "gad7_score", assessmentData.gad7Score.toString())
                addFormField(writer, boundary, "questionnaire_score", assessmentData.questionnaireScore.toString())
                ...
                // GAD7 CSV
                assessmentData.gad7CsvFile?.let { file ->
                    if (file.exists()) {
                        progressStep++
                        Log.d("UPLOAD_DEBUG", "Uploading GAD7 CSV...")
                        addFileField(writer, outputStream, boundary, "gad_csv", file, "text/csv")
                ...
                Log.e("UPLOAD_DEBUG", "Upload exception type: ${e.javaClass.simpleName}")
                Log.e("UPLOAD_DEBUG", "Upload exception message: ${e.message}")
...
//    suspend fun uploadAnonymousAssessment(
//        assessmentData: com.example.unifiedapp.ui.data.AssessmentData,
//        callback: UploadCallback
//    ) {
//        withContext(Dispatchers.IO) {
//            try {
...
//
//
//                callback.onProgress(5, "Preparing anonymous data upload...")
//                Log.d(TAG, "Starting anonymous assessment upload: ${assessmentData.anonymousId}")
//
//                callback.onProgress(10, "Building upload request...")
//
//                val requestBuilder = MultipartBody.Builder()
//                    .setType(MultipartBody.FORM)
//
//                requestBuilder.addFormDataPart("anonymous_id", assessmentData.anonymousId)
//                requestBuilder.addFormDataPart("age", assessmentData.age.toString())
//                requestBuilder.addFormDataPart("assessment_type", assessmentData.assessmentType)
//
//                callback.onProgress(20, "Adding video file...")
//
//                assessmentData.videoFile?.let { videoFile ->
//                    if (videoFile.exists()) {
//                        Log.d(TAG, "Adding video: ${videoFile.name} (${videoFile.length()} bytes)")
...
//                }
//
//                callback.onProgress(40, "Adding facial analysis data...")
//
//                assessmentData.auCsvFile?.let { auCsvFile ->
//                    if (auCsvFile.exists()) {
//                        Log.d(TAG, "Adding AU CSV: ${auCsvFile.name} (${auCsvFile.length()} bytes)")
//                        requestBuilder.addFormDataPart(
//                            "au_csv",
//                            auCsvFile.name,
//                            auCsvFile.asRequestBody("text/csv".toMediaType())
//                        )
//                    } else {
//                        Log.w(TAG, "AU CSV file not found: ${auCsvFile.absolutePath}")
//                    }
//                }
//
//                callback.onProgress(60, "Adding questionnaire data...")
//
//
//
//                requestBuilder.addFormDataPart("questionnaire_data", assessmentData.questionnaireData)
//
//                callback.onProgress(80, "Uploading to research server...")
//
//                val request = Request.Builder()
//                    .url("$baseUrl/api/student/upload-assessment")
//                    .post(requestBuilder.build())
//                    .build()
//
//                Log.d(TAG, "Sending anonymous assessment data to server...")
//
//                val response = client.newCall(request).execute()
//                val responseBody = response.body?.string()
//
//                callback.onProgress(95, "Processing server response...")
//
//                if (response.isSuccessful) {
//                    callback.onProgress(100, "Upload complete!")
//
//                    Log.d(TAG, "Anonymous assessment uploaded successfully!")
//                    Log.d(TAG, "Server response: $responseBody")
//
//                    withContext(Dispatchers.Main) {
//                        callback.onSuccess(
//                            "Anonymous assessment uploaded successfully!\n\n" +
//                                    "Anonymous ID: ${assessmentData.anonymousId}\n\n" +
//                                    "Your privacy is protected - no personal information was shared.\n" +
    ...
    suspend fun sendReportToParentEmail(
        ...
        return withContext(Dispatchers.IO) {
            ...
            try {
                ...
                val responseBody = try {
                    java.io.BufferedReader(java.io.InputStreamReader(connection.inputStream)).use {
                        it.readText()
                    }
                } catch (e: Exception) {
                    try {
                        java.io.BufferedReader(java.io.InputStreamReader(connection.errorStream)).use {
                            it.readText()
                        }
                    } catch (e2: Exception) {
                        ""
